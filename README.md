# cobra (HiveOS BTX miner)

`cobra` is a repackaging of matador-miner v0.9.25 (MIT license, unmodified
binary) as a HiveOS custom miner named `cobra`, with a fixed `h-stats.sh`
(see its header comment). `LICENSE` is matador-miner's original MIT license.
