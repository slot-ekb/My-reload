#!/usr/bin/env python3
# Прокси proxygpu: 1 соединение к ноде, много GPU-воркеров локально.
...
algo_jobs = {}  # заданий по алгоритмам {progpow:.., randomx:.., cuckoo:..}
    ...
    while True:
        try:
            ...
            w.write((json.dumps({"id": "0", "jsonrpc": "2.0", "method": "login",
                                 "params": {"login": HOSTNAME + "-gpu", "pass": "", "agent": "proxygpu"}}) + "\n").encode())
            ...
            async for line in r:
                ...
                if p is not None:
                    ...
                    a = p.get("algorithm", "?")
                    algo_jobs[a] = algo_jobs.get(a, 0) + 1
                    await broadcast_job()
...
async def stats_loop():
    while True:
        ...
        log(f"СТАТ воркеров={len(workers)} задания_получ={jobs_rx} задания_розд={jobs_tx} "
            f"шары_получ={sub_rx} шары_отосл={sub_tx} алго={algo} | по_алго[{brk}]")

async def keepalive_loop():
    # keepalive на ноду каждые 30с: коннект живой + время на ноде тикает как у прямых майнеров
    while True:
...
async def main():
    ...
    log(f"ПРОКСИ proxygpu: воркеры -> :{LISTEN_PORT} | нода -> {NODE_HOST}:{NODE_PORT}")
    async with srv:
